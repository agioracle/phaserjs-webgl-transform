...
function buildCanvasValue(): t.MemberExpression {
  // Douyin: the tt-adapter creates the on-screen canvas and, per the official
  // Adapter docs, mounts it on the global `window` as `window.canvas` — this is
  // the canonical reference and works in BOTH the developer tool and on real
  // devices. (GameGlobal.canvas is only populated on real devices; the devtools
  // inject() branch defines properties on the real `window` and never sets
  // GameGlobal.canvas.) Phaser must reuse this canvas — otherwise
  // document.createElement('canvas') / tt.createCanvas() returns a NEW
  // OFF-screen canvas and nothing renders.
...
function mergeObjectProperties(
  ...
  if (existingProps.has('type')) {
    ...
    if (!isWebGLType(typeProp.value as t.Node)) {
      warnings.push(
        'Renderer type is not Phaser.WEBGL. Overriding to Phaser.WEBGL for Douyin Mini-Game compatibility.'
      );
