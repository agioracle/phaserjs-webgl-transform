...
describe('CLI entry point', () => {
  ...
  it('has correct name and description', () => {
    ...
    expect(program.name()).toBe('phaser-tt');
    expect(program.description()).toMatch(/douyin/i);
  });
  ...
  it('build command defaults --target to tt', () => {
    const program = createProgram();
    const buildCmd = program.commands.find((c) => c.name() === 'build')!;
    const targetOption = buildCmd.options.find((o) => o.long === '--target');
    expect(targetOption).toBeDefined();
    expect(targetOption!.defaultValue).toBe('tt');
  });

  it('dispatches init command', async () => {
    const program = createProgram();
    await program.parseAsync(['node', 'phaser-tt', 'init']);
    expect(initCommand).toHaveBeenCalledTimes(1);
  });

  it('dispatches build command with --cdn option', async () => {
    const program = createProgram();
    await program.parseAsync(['node', 'phaser-tt', 'build', '--cdn', 'https://mycdn.com']);
