...
import type { PhaserTtConfig } from '../../src/utils/config.js';
...
const FULL_CONFIG_WITH_DEFAULTS: PhaserTtConfig = {
  appid: 'tt1234567890abcdef',
...
describe('loadConfig', () => {
  ...
  it('loads a valid config from default path and fills defaults', () => {
    const defaultPath = path.join(process.cwd(), 'phaser-tt.config.json');
  ...
  it('accepts a Douyin appid that does not start with "wx"', () => {
    const ttAppid = { ...VALID_CONFIG, appid: 'tt9f8e7d6c5b4a3210' };
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify(ttAppid));

    const result = loadConfig();
    expect(result.appid).toBe('tt9f8e7d6c5b4a3210');
  });
