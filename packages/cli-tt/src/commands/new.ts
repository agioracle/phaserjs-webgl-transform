    ...
    if (parent === dir) {
      throw new Error(
        'Could not find monorepo root (no pnpm-workspace.yaml found). ' +
          'phaser-tt new must be run within the monorepo.'
      );
    }
    dir = parent;
  }
...
/**
 * Filter function for fs.cpSync — excludes node_modules, dist-tt, dist-h5, .DS_Store, package-lock.json.
 */
...
export async function newCommand(
  ...
  const answers = await inquirer.prompt([
    {
      ...
      validate: (input: string) => {
        if (!input || input.trim() === '') return 'AppID is required';
        return true;
      },
    },
    {
  ...
  const templateName = answers.orientation === 'landscape' ? 'example-tt-landscape' : 'example-tt-portrait';
  ...
  // 2. phaser-tt.config.json — replace appid, orientation, cdn
  const configPath = path.join(targetDir, 'phaser-tt.config.json');
