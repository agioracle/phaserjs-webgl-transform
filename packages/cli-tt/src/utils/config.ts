...
export interface PhaserTtConfig {
  appid: string;
...
export function loadConfig(configPath?: string): PhaserTtConfig {
  const resolvedPath = configPath ?? path.join(process.cwd(), 'phaser-tt.config.json');
  ...
  // Douyin appids do not use the WeChat "wx" prefix, so only require a
