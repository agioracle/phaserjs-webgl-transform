# phaser-tt-example (landscape)

Example Phaser.js 3.x FlappyBird game (landscape) for Douyin Mini-Game, built with `phaser-tt`.

This project serves as both a development/test target and the source template for `phaser-tt new` (landscape orientation).
...
4. **GameScene** — FlappyBird 游戏（小鸟、管道、计分），并行预下载 game-over 分包
5. **GameOverScene** — 关卡结算（最终得分），点击返回 MenuScene
...
| 分包 | 内容 | 说明 |
|------|------|------|
| 主包 | game.js + tt-adapter + game-bundle.js | 闪屏 + BootScene |
...
分包配置在 `phaser-tt.config.json` 的 `subpackages` 字段中声明。
...
src/
  ...
  scenes/
    ...
    GameScene.js       FlappyBird game scene (preloads game-over subpackage)
    GameOverScene.js   Round-end result screen (final score)
  utils/
...
public/
  ...
  remote-assets/       Remote assets (loaded from CDN, never bundled)
    images/
    audio/
phaser-tt.config.json  Build configuration (includes subpackages)
```
