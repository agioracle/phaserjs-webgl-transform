...
2. **资源管线**（`generateBundle` 钩子）：
   - `remoteAssetsDir` 目录下的资源自动标记为远程（不复制到输出目录）
...
| **远程资源目录分离** | `remoteAssetsDir` 下的资源在 manifest 中自动标记为 `remote: true`，构建时不复制到输出目录，由用户手动上传到 CDN。避免大资源撑大小游戏包体积 |
...
1. 将 `dist-wx/` 目录导入微信开发者工具
2. 将 `public/remote-assets/` 中的文件上传到 CDN 对应路径（保持目录结构），例如：
   - `public/remote-assets/audio/bgm.mp3` → `https://cdn.example.com/remote-assets/audio/bgm.mp3`

> **注意**：`asset-manifest.json` 中 `remote: true` 的资源，运行时会通过 `cdnBase + 资源路径` 拼接 URL 下载。请确保 CDN 上的文件路径与 `remote-assets/` 内的目录结构一致。
